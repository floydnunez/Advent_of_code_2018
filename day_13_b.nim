...
let file_name = "d13_input_reddit.txt"
...
var vehicle_count = vehicle_pos.len

proc find_only_car(mapa: seq[seq[Cell]]) =
    var which_x = -1
    var which_y = -1
    for ii in 0 .. height - 1:
        for jj in 0 .. width - 1:
            if mapa[ii][jj].vehicle != ' ':
                echo "found one! at ", jj, " , ", ii
                which_x = jj
                which_y = ii
...
while true: 
    ...
    for ii in 0 .. height - 1:
        for jj in 0 .. width - 1:
            ...
            if map[ii][jj].vehicle == '<':
                ...
                if next_map[ii][jj - 1].vehicle != ' ':
                    echo "out tick collision: ", map[ii][jj].vehicle_index, " + ", next_map[ii][jj - 1].vehicle_index, " pos: (", jj - 1, ", ", ii, ")" 
                    next_map[ii][jj - 1].vehicle = ' '
                    eliminated = true
                    vehicle_count -= 2
                elif map[ii][jj - 1].vehicle != ' ':
                    echo "in  tick collision: ", map[ii][jj].vehicle_index, " + ", map[ii][jj - 1].vehicle_index, " pos: (", jj - 1, ", ", ii, ")" 
                    next_map[ii][jj - 1].vehicle = ' '
                    eliminated = true
                    vehicle_count -= 1
                if not eliminated:
            ...
            if map[ii][jj].vehicle == '^':
                ...
                vehicle_pos[map[ii][jj].vehicle_index].v = vehicle_next
                if next_map[ii - 1][jj].vehicle != ' ':
                    echo "out tick collision: ", map[ii][jj].vehicle_index, " + ", next_map[ii - 1][jj].vehicle_index, " pos: (", jj, ", ", ii - 1, ")" 
                    next_map[ii - 1][jj].vehicle = ' '
                    eliminated = true
                    vehicle_count -= 2
                elif map[ii - 1][jj].vehicle != ' ':
                    echo "in  tick collision: ", map[ii][jj].vehicle_index, " + ", map[ii - 1][jj].vehicle_index, " pos: (", jj, ", ", ii - 1, ")" 
                    next_map[ii - 1][jj].vehicle = ' '
                    eliminated = true
                    vehicle_count -= 1
                if not eliminated:
    ...
    if vehicle_count <= 1:
        echo "NO MORE CARTS!"
        echo vehicle_pos
        find_only_car(next_map)
        quit(1)
    else:
        echo "current_vehicles: ", vehicle_pos
        echo vehicle_pos
